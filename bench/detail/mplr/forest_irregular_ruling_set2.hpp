...
class forest_irregular_ruling_set2 //this is for forest
{
	...
	void calculate_adj_arr(std::vector<std::uint64_t>& s, std::vector<std::int64_t>& r, std::vector<std::uint32_t>& targetPEs, kamping::Communicator<>& comm, karam::mpi::GridCommunicator& grid_comm, std::uint64_t node_offset)
	{
		...
		auto recv = comm.alltoallv(kamping::send_buf(send_edges), kamping::send_counts(num_packets_per_PE));
	...
	void start(kamping::Communicator<>& comm, karam::mpi::GridCommunicator& grid_comm, timer& timer)
	{
		...
		while (any_PE_has_work(comm, work_left))
		{
			...
			std::vector<packet> recv_buffer = comm.alltoallv(kamping::send_buf(out_buffer), kamping::send_counts(num_packets_per_PE));
		...
		auto recv = comm.alltoallv(kamping::send_buf(requests), kamping::send_counts(num_packets_per_PE), kamping::recv_counts_out());
		...
		std::vector<std::uint64_t> recv_answers = comm.alltoallv(kamping::send_buf(recv_requests), kamping::send_counts(recv.extract_recv_counts()));
		std::fill(num_packets_per_PE.begin(), num_packets_per_PE.end(), 0);
		for (std::uint64_t i = 0; i < local_rulers.size(); i++)
		{
			std::int32_t targetPE = mst_PE[local_rulers[i]];
		...
		*/
		...
		auto recv_request = comm.alltoallv(kamping::send_buf(request), kamping::send_counts(num_packets_per_PE), kamping::recv_counts_out());
		...
		auto recv_answers_buffer = comm.alltoallv(kamping::send_buf(answers), kamping::send_counts(recv_request.extract_recv_counts()));
		timer.switch_category("local_work");
		std::fill(num_packets_per_PE.begin(), num_packets_per_PE.end(), 0);
