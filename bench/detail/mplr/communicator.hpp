...
#include "detail/mplr/karam/utils/containers.hpp"
#include "timer.hpp"

template <typename packet>
static std::vector<packet> alltoall(timer& timer,
...
static std::vector<answer> request_reply(timer& timer,
                                         std::vector<request>& requests,
                                         std::vector<std::int32_t>& send_counts,
                                         std::function<answer(const request)>& lambda,
                                         kamping::Communicator<>& comm,
                                         karam::mpi::GridCommunicator& grid_comm,
                                         bool grid) {
  if (grid)
    return request_reply_grid<request, answer>(timer, requests, send_counts, lambda, comm,
                                               grid_comm);
  else
    return request_reply_normal<request, answer>(timer, requests, send_counts, lambda,
                                                 comm);
}
...
static std::vector<answer> request_reply(
    ...
    bool grid,
    bool aggregate,
    std::function<std::uint64_t(const request)>& request_assingment) {
  if (grid) {
    if (aggregate)
      return request_reply_aggregate_grid<request, answer>(
          timer, requests, request_assingment, send_counts, lambda, comm, grid_comm);
    else
      return request_reply_grid<request, answer>(timer, requests, send_counts, lambda,
                                                 comm, grid_comm);
  } else {
    if (aggregate)
      return request_reply_aggregate_normal<request, answer>(
          timer, requests, request_assingment, send_counts, lambda, comm);
    else
      return request_reply_normal<request, answer>(timer, requests, send_counts, lambda,
                                                   comm);
  }
...
template <typename packet>
static std::vector<packet> allgatherv(timer& timer,
                                      std::vector<packet>& send_buf,
                                      kamping::Communicator<>& comm,
                                      karam::mpi::GridCommunicator grid_comm,
                                      bool grid) {
...
template <typename packet>
static std::vector<packet> allgatherv_grid(timer& timer,
                                           std::vector<packet> send_buf,
                                           kamping::Communicator<>& comm,
                                           karam::mpi::GridCommunicator grid_comm) {
...
template <typename packet>
static std::vector<packet> allgatherv_normal(timer& timer,
                                             std::vector<packet> send_buf,
                                             kamping::Communicator<>& comm) {
  timer.switch_category("communication");
  std::vector<packet> to_return;
  comm.allgatherv(kamping::send_buf(send_buf),
                  kamping::recv_buf<kamping::resize_to_fit>(to_return));
  timer.switch_category("local_work");
  return to_return;
}

template <typename packet>
static std::vector<packet> alltoall_normal(timer& timer,
                                           std::vector<packet>& send_buf,
                                           std::vector<std::int32_t>& send_counts,
                                           kamping::Communicator<>& comm) {
  timer.switch_category("communication");
...
template <typename packet>
static std::vector<packet> alltoall_grid(timer& timer,
                                         std::vector<packet>& send_buf,
                                         std::vector<std::int32_t>& send_counts,
                                         kamping::Communicator<>& comm,
                                         karam::mpi::GridCommunicator& grid_comm) {
  timer.switch_category("communication");
  auto recv = my_grid_all_to_all(send_buf, send_counts, grid_comm, comm);
  timer.switch_category("local_work");
...
template <typename request, typename answer>
static std::vector<answer> request_reply_aggregate_grid(
    timer& timer,
    std::vector<request> requests,
    std::function<std::uint64_t(const request)> request_assingment,
    std::vector<std::int32_t> send_counts,
  ...
  std::vector<std::int32_t> send_counts_row =
      std::vector<std::int32_t>(grid_comm.row_comm().size(), 0);
  for (const auto& [key, value] : request_map) {
    std::int32_t targetPE =
        grid_comm.proxy_col_index(static_cast<std::size_t>(value.targetPE));
    send_counts_row[targetPE]++;
  }
  std::vector<std::int32_t> send_displacements_row =
      std::vector<std::int32_t>(grid_comm.row_comm().size() + 1);
  send_displacements_row[0] = 0;
  for (std::uint32_t p_row = 0; p_row < grid_comm.row_comm().size(); p_row++)
    send_displacements_row[p_row + 1] =
        send_displacements_row[p_row] + send_counts_row[p_row];
  std::fill(send_counts_row.begin(), send_counts_row.end(), 0);
  struct send_request {
    request request_value;
    std::uint64_t key;
  };
  karam::utils::default_init_vector<karam::mpi::IndirectMessage<send_request>>
      send_buf_row(send_displacements_row[grid_comm.row_comm().size()]);
  ;
  for (const auto& [key, value] : request_map) {
    std::int32_t targetPE =
        grid_comm.proxy_col_index(static_cast<std::size_t>(value.targetPE));
    std::uint64_t packet_index =
        send_displacements_row[targetPE] + send_counts_row[targetPE]++;

    send_buf_row[packet_index] = karam::mpi::IndirectMessage<send_request>(
        static_cast<std::uint32_t>(comm.rank()),
        static_cast<std::uint32_t>(value.targetPE), {value.request_value, key});
  }

  timer.switch_category("communication");
  auto mpi_result_rowwise = grid_comm.row_comm().alltoallv(
      kamping::send_buf(send_buf_row), kamping::send_counts(send_counts_row),
  ...
  for (std::uint64_t i = 0; i < rowwise_recv_buf.size(); i++) {
    if (!request_map.contains(rowwise_recv_buf[i].payload().key)) {
      request_map[rowwise_recv_buf[i].payload().key] = {
          rowwise_recv_buf[i].payload().request_value,
          static_cast<std::int32_t>(rowwise_recv_buf[i].get_destination())};
    }
  }
  /*
  ...
  std::cout << std::endl;*/
  std::vector<std::int32_t> send_counts_col =
      std::vector<std::int32_t>(grid_comm.col_comm().size(), 0);
  for (const auto& [key, value] : request_map) {
    std::int32_t targetPE =
        grid_comm.proxy_row_index(static_cast<std::size_t>(value.targetPE));
    send_counts_col[targetPE]++;
  }
  std::vector<std::int32_t> send_displacements_col =
      std::vector<std::int32_t>(grid_comm.col_comm().size() + 1);
  send_displacements_col[0] = 0;
  for (std::uint32_t p_col = 0; p_col < grid_comm.col_comm().size(); p_col++)
    send_displacements_col[p_col + 1] =
        send_displacements_col[p_col] + send_counts_col[p_col];
  std::fill(send_counts_col.begin(), send_counts_col.end(), 0);
  karam::utils::default_init_vector<karam::mpi::IndirectMessage<send_request>>
      send_buf_col(send_displacements_col[grid_comm.col_comm().size()]);
  for (const auto& [key, value] : request_map) {
    std::int32_t targetPE =
        grid_comm.proxy_row_index(static_cast<std::size_t>(value.targetPE));
    std::uint64_t packet_index =
        send_displacements_col[targetPE] + send_counts_col[targetPE]++;

    send_buf_col[packet_index] = karam::mpi::IndirectMessage<send_request>(
        static_cast<std::uint32_t>(comm.rank()),
        static_cast<std::uint32_t>(value.targetPE), {value.request_value, key});
  }
  timer.switch_category("communication");
  auto colwise_recv = grid_comm.col_comm().alltoallv(
      kamping::send_buf(send_buf_col), kamping::send_counts(send_counts_col), kamping::recv_counts_out());
  timer.switch_category("local_work");
  ...
  for (std::uint64_t i = 0; i < colwise_recv_buf.size(); i++)
    send_buf_col_back[i] = {lambda(colwise_recv_buf[i].payload().request_value),
                            colwise_recv_buf[i].payload().key};

  timer.switch_category("communication");
  auto colwise_recv_buf_back =
      grid_comm.col_comm()
          .alltoallv(kamping::send_buf(send_buf_col_back),
                     kamping::send_counts(colwise_recv.extract_recv_counts()));
  timer.switch_category("local_work");
  /*
  ...
  for (std::uint64_t i = 0; i < answers_row.size(); i++)
    answers_row[i] = {answer_map[rowwise_recv_buf[i].payload().key],
                      rowwise_recv_buf[i].payload().key};

  timer.switch_category("communication");
  auto rowwise_recv_buf_back =
      grid_comm.row_comm()
          .alltoallv(kamping::send_buf(answers_row),
                     kamping::send_counts(mpi_result_rowwise.extract_recv_counts()));
  timer.switch_category("local_work");
...
template <typename request, typename answer>
static std::vector<answer> request_reply_aggregate_normal(
    timer& timer,
    std::vector<request> requests,
    std::function<std::uint64_t(const request)> request_assingment,
    std::vector<std::int32_t> send_counts,
  ...
  for (const auto& [key, value] : request_map) {
    std::uint32_t targetPE = value.targetPE;
    std::uint64_t packet_index =
        send_displacements[targetPE] + num_packets_per_PE[targetPE]++;
    // std::cout << comm.rank() << " requestet " << value.request_value << std::endl;
    send_requests[packet_index] = {value.request_value, key};
  }

  // hier kann auch request reply benutzt werden. Dann annahme; recv_answers in selber
  // reihenfolge wie send_requests
  timer.switch_category("communication");
  auto recv = comm.alltoallv(kamping::send_buf(send_requests),
                             kamping::send_counts(num_packets_per_PE), kamping::recv_counts_out());
  timer.switch_category("local_work");

  std::vector<send_request> recv_requests = recv.extract_recv_buffer();
  if (recv_requests.size() > 10 * send_requests.size())
    std::cout << comm.rank() << " sendet " << send_requests.size()
              << " requests und bekommt " << recv_requests.size() << " requests "
              << std::endl;
  ...
  timer.switch_category("communication");
  auto recv_answers = comm.alltoallv(kamping::send_buf(send_answers),
                                     kamping::send_counts(recv.extract_recv_counts()));
  timer.switch_category("local_work");
...
template <typename request, typename answer>
static std::vector<answer> request_reply_normal(
    timer& timer,
    std::vector<request>& requests,
    std::vector<std::int32_t>& send_counts,
    std::function<answer(const request)> lambda,
  ...
  timer.switch_category("communication");
  auto recv =
      comm.alltoallv(kamping::send_buf(requests), kamping::send_counts(send_counts),
                     kamping::recv_counts_out());
  timer.switch_category("local_work");
  ...
  timer.switch_category("communication");
  std::vector<answer> recv_answers = comm.alltoallv(
      kamping::send_buf(answers), kamping::send_counts(recv.extract_recv_counts()));
  timer.switch_category("local_work");
...
template <typename request, typename answer>
static std::vector<answer> request_reply_grid(timer& timer,
                                              std::vector<request>& requests,
                                              std::vector<std::int32_t>& send_counts,
                                              std::function<answer(const request)> lambda,
                                              kamping::Communicator<>& comm,
                                              karam::mpi::GridCommunicator grid_comm) {
  timer.switch_category("communication");
  auto recv_request = my_grid_all_to_all(requests, send_counts, grid_comm, comm);
  timer.switch_category("local_work");
  ...
  timer.switch_category("communication");
  auto recv_indirect_answers =
      my_grid_all_to_all(answers, recv_counts, grid_comm, comm);
  timer.switch_category("local_work");
...
request_reply_normal<std::uint64_t,std::uint64_t>(requests, send_counts, lambda, comm);
