...
#include "forest_irregular_optimized_ruling_set.hpp"
#include "forest_irregular_pointer_doubling.hpp"
...
public:
  forest_regular_optimized_ruling_set(std::uint64_t comm_rounds,
  ...
  void add_timer_info(std::string info) { this->info = "\"" + info + "\""; }

  void start(std::vector<std::uint64_t> s,
    ...
    timer timer("graph_umdrehen", categories, "local_work",
                "forest_regular_optimized_ruling_set");
    ...
    calculate_send_displacements_and_reset_num_packets_per_PE(send_displacements,
                                                              num_packets_per_PE);
    ...
    std::vector<edge> local_edges =
        alltoall(timer, send_edges, num_packets_per_PE, comm, grid_comm, grid);
    ...
    for (std::uint64_t i = 0; i < local_roots.size(); i++) {
      ...
      if (unmask(bounds[maybe_ruler + 1]) - unmask(bounds[maybe_ruler]) > 0) {
        ...
        // std::cout << maybe_ruler + node_offset<< " ist auf jeden fall ruler" <<
        // std::endl;
      } else {
    ...
    // std::cout << rank << " schickt so so viele packets pro iteration " <<
    // num_packages_per_iteration << ", bei insgesamt " << edges.size() << " edges und "
    // << local_roots.size() << " local_roots und davon " << definitely_rulers.size() << "
    // roots mit grad > 1 und " << num_local_vertices << " num_local_vertices"<<
    // std::endl;
    ...
    while (num_packages_sent < num_packages_per_iteration) {
      ...
      // std::cout << rank << " startet wave mit folgendem ruler: " << ruler + node_offset
      // << std::endl;
    ...
    for (std::uint32_t iteration = 0; iteration < comm_rounds + 2; iteration++) {
      // std::cout << rank << " in  iteration " << iteration << " hat out_buffer size von
      ...
      */
      std::vector<packet> recv_buffer =
          alltoall(timer, out_buffer, num_packets_per_PE, comm, grid_comm, grid);
      /*
      std::cout << rank << " in iteration " << iteration << " with following
      recv_buffer:"; for (int i = 0; i < recv_buffer.size(); i++) std::cout <<
      packet_to_string(recv_buffer[i]) << ", "; std::cout << std::endl;
      */
      ...
      for (packet& packet : recv_buffer) {
        ...
        if (is_ruler(local_index)) {
          ...
          packets_to_forward_out_degree +=
              unmask(bounds[local_index + 1]) - unmask(bounds[local_index]);
        }
      }
      // jetzt noch ruler in die queue hinzufügen, damit queue genug elemente hat

      // std::cout << rank << " in iteration " << iteration << " queue hat out degree " <<
      // packets_to_forward_out_degree << " und ich brauche " <<
      // num_packages_per_iteration << std::endl;
      ...
      if (packets_to_forward_out_degree < num_packages_per_iteration) {
        ...
        while (rulers_to_send_packages_in_this_iteration_out_degree <
               num_packages_per_iteration - packets_to_forward_out_degree) {
          std::uint64_t ruler = get_next_ruler();

          if (ruler == -1)
            break;

          // std::cout << rank << " in iteration " << iteration << " fügt folgenden ruler
          // in queue " << ruler << std::endl;
          local_rulers.push_back(ruler);
          mark_as_ruler(ruler);
          rulers_to_send_packages_in_this_iteration_out_degree +=
              unmask(bounds[ruler + 1]) - unmask(bounds[ruler]);
          rulers_to_send_packages_in_this_iteration.push_back(ruler);
        }
      }
      ...
      while (packets_to_forward_out_degree > degree) {
        ...
        // std::cout << rank << " in iteration " << iteration << " popped folgendes
        // element aus queue " << packet_to_string(packet) << std::endl;
        ...
        packets_to_forward_out_degree -=
            unmask(bounds[local_index + 1]) - unmask(bounds[local_index]);
        for (std::uint64_t i = unmask(bounds[local_index]);
             i < unmask(bounds[local_index + 1]); i++) {
          std::int32_t targetPE = calculate_targetPE(edges[i]);
          num_packets_per_PE[targetPE]++;
        }
      }
      ...
      for (packet& packet : packets) {
        std::uint64_t local_index = packet.destination - node_offset;
        // std::cout << rank << " jetzt wird packet geschickt mit node " << local_index <<
        // std::endl;
        for (std::uint64_t i = unmask(bounds[local_index]);
             i < unmask(bounds[local_index + 1]); i++) {
          std::int32_t targetPE = calculate_targetPE(edges[i]);
      ...
      for (std::uint64_t local_index : rulers_to_send_packages_in_this_iteration) {
        // std::cout << rank << " jetzt wird neuer ruler geschickt naemlich " <<
    ...
    */
    ...
    std::uint64_t node_offset_rec =
        comm.exscan(kamping::send_buf((std::uint64_t)local_rulers.size()),
                    kamping::op(kamping::ops::plus<>()))[0];
    ...
    std::function<std::uint64_t(const std::uint64_t)> lambda =
        [&](std::uint64_t request) {
          return map_ruler_to_its_index[request - node_offset] + node_offset_rec;
        };
    std::function<std::uint64_t(const std::uint64_t)> request_assignment =
        [](std::uint64_t request) { return request; };

    std::vector<std::uint64_t> recv_answers =
        request_reply(timer, requests, num_packets_per_PE, lambda, comm, grid_comm, grid,
                      aggregate, request_assignment);
    ...
    /*
    for (int i = 0; i < local_rulers.size(); i++)
            std::cout << i + prefix_sum_num_vertices_per_PE[rank] << " s_rec:" << s_rec[i]
    << ", r_rec:" << r_rec[i] << std::endl;
    */
    ...
    if (num_global_vertices_rec / size > 10000) {
      forest_irregular_optimized_ruling_set recursion(comm_rounds, num_iterations - 1,
                                                      grid, aggregate);
      recursion.start(s_rec, r_rec, targetPEs_rec, node_offset_rec,
                      num_global_vertices_rec, comm, grid_comm,
                      local_rulers_global_index);

      recursive_global_index = recursion.result_root;
      recursive_r = recursion.result_dist;
    } else {
      forest_irregular_pointer_doubling recursion(
          s_rec, r_rec, targetPEs_rec, node_offset_rec, num_global_vertices_rec,
          local_rulers_global_index, grid, aggregate);
    ...
    std::function<answer(const std::uint64_t)> lambda2 = [&](std::uint64_t request) {
      answer answer;
      answer.global_root_index =
          recursive_global_index[map_ruler_to_its_index[request - node_offset]];
      answer.distance = recursive_r[map_ruler_to_its_index[request - node_offset]];
      return answer;
    };

    std::vector<answer> recv_answers_buffer =
        request_reply(timer, request, num_packets_per_PE, lambda2, comm, grid_comm, grid,
                      aggregate, request_assignment);
    ...
    for (std::uint64_t i = 0; i < num_local_vertices; i++) {
      ...
      // std::cout << i + node_offset << " hat dist " << result_dist[i] << " von root " <<
      // result_root[i] << std::endl;
    }
  ...
  std::uint64_t get_next_ruler() {
    ...
    while (ruler_index < num_local_vertices &&
           (is_reached(ruler_index) || is_ruler(ruler_index) ||
            (unmask(bounds[ruler_index + 1]) == unmask(bounds[ruler_index]))))
      ruler_index++;
  ...
  std::string packet_to_string(packet packet) {
    return "(" + std::to_string(packet.ruler_source) + "," +
           std::to_string(packet.destination) + "," + std::to_string(packet.distance) +
           ")";
  }
  ...
  bool is_ruler(std::uint64_t local_index) { return is_marked(bounds[local_index], 0); }

  void mark_as_reached(std::uint64_t local_index) {
    bounds[local_index] = mark(bounds[local_index], 1);
  }

  bool is_reached(std::uint64_t local_index) { return is_marked(bounds[local_index], 1); }

  bool is_leaf(std::uint64_t local_index) {
    return unmask(bounds[local_index]) == unmask(bounds[local_index + 1]);
  }

  std::uint64_t unmask(std::uint64_t value) { return value & 0xfffffffffffffff; }
  ...
  void calculate_send_displacements_and_reset_num_packets_per_PE(
