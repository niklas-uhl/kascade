...
auto main(int argc, char* argv[]) -> int {
  {
    ...
    //  kamping::measurements::timer().synchronize_and_start("cc");
    //  algo->run();
    //  kamping::measurements::timer().stop_and_append();

    //  kamping::measurements::timer().synchronize_and_start("extract_component_count");
    //  auto num_components = algo->get_num_components();
    //  kamping::measurements::timer().stop_and_append();
    //  SPDLOG_LOGGER_INFO(spdlog::get("root"), "Found {} components in {} secs.",
    //                     num_components, stopwatch);
